...
Then("I should not be able to access restricted pages", async () => {
  loginP = new LoginPage(pageF.page);
  await pageF.page.goto("http://localhost:5000/weather");
  let url = await pageF.page.url();
  await expect(url).toContain("login");
  await loginP.checkLoginButtonDisabled();
  let error = await loginP.getErrorMessage();
  //expect error to be visible
  await expect(error).toContain(
    "You need to be logged in to access this page."
  );
  await pageF.page.goto("http://localhost:5000/files");
  url = await pageF.page.url();
  await expect(url).toContain("login");
  await loginP.checkLoginButtonDisabled();
  error = await loginP.getErrorMessage();
  //expect error to be visible
  await expect(error).toContain(
    "You need to be logged in to access this page."
  );
  await pageF.page.goto("http://localhost:5000/chat");
  url = await pageF.page.url();
  await expect(url).toContain("login");
  await loginP.checkLoginButtonDisabled();
  error = await loginP.getErrorMessage();
  ...
  await pageF.page.goto("http://localhost:5000/admin");
  //on this route you are redirected to / if you are not logged in as admin
    url = await pageF.page.url();
