...
db_host = os.getenv("DB_CLUSTER", "mongo")  # Host should match the service name in Docker Compose
db_name = os.getenv("DB_NAME", "default_db")
db_users_collection = os.getenv("DB_USERS_COLLECTION", "Users")
...
print(f"Users Collection: {db_users_collection}")

# MongoDB URI
mongo_uri = f"mongodb://{db_username}:{db_password}@{db_host}:27017/?authSource=admin"
try:
    # Initialize MongoDB client
    client = MongoClient(mongo_uri)
    db = client[db_name]  # Access the specified database
    users_collection = db[db_users_collection]  # Access the specified collection
    settings_collection = db['settings']  # Access the settings collection
    messages_collection = db['messages']  # Access the messages collection
    logs_collection = db['logs']  # Access the logs collection
    print("Database connection and collection access successful.")
...
def weather_result():
    ...
    try:
        geocode_response = requests.get(
            geocode_url, params={'q': city,
                                 'key': "a860e9fa8d38407d980de974ea2d8315"}
        )
